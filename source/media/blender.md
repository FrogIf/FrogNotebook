...
导入模型时, 选择工程文件后, 会自动进入到文件内部, 这时模型在Object目录下. 如果想导入集合, 需要选择Collection目录下的目标.
...
毛发系统, 如果使用物体作为毛发对象, 一般都需要调整方向, 需要注意有一定的步骤技巧(局部检查, 正Z正X):

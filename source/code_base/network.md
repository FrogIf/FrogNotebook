...
![image](img/mac_package.png)

### 3.2 ARP

ARP协议是指定IP地址, 获取该IP地址处设备的MAC地址. 当知道接收方的IP地址后, 通过广播的方式, 询问局域网中的其他设备, "IP:xxx的MAC地址是多少?", 广播出去之后, 谁的IP是报文中的IP, 谁就回答. 此外并不是每次发送数据, 都需要先进行ARP请求, 机器会将ARP缓存在本地.

ARP协议包示意图如下:

![image](img/ARP_package.png)

> 发送请求时, 广播的目的mac是: ff:ff:ff:ff:ff:ff

> ARP到底属于网络层协议还是数据链路层是有争议的
> ![image](img/arp_protocol.png)

### 3.3 交换机

一个局域网中, 如果有多台主机设备, 如果通过集线器进行数据转发会有问题. 每一个数据包都广播到其他线路上去, 让主机判断目的mac地址是不是自己的, 导致而不是自己的就丢弃, 这样资源浪费. 这时就需要一种设备, 通过mac地址确定应该转发到哪个线路的端口上去, 这个设备就是**交换机**.

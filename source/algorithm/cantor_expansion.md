...
假设有这么一个序列: [a, b, c, d]. 然后, 列出这个序列的所有全排列, 如下:
...
首先, 我们只看开头第一个字符, 可以看到, 以a开头的序列有6个, 以b开头的也有6个, 以此类推, 每一个开头的都有6个. 这是因为, 第一位固定的情况下, 其余三位的排列个数就是3!=6. 这样, 可以总结出一个关于**某个字符占据索引范围**的规律:
